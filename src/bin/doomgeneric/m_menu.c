...
{
    printf("[doom] M_QuitResponse key=%d('%c'), key_menu_confirm=%d('%c'), key_menu_abort=%d('%c')\n",
           key, key, key_menu_confirm, key_menu_confirm, key_menu_abort, key_menu_abort);
    if (key != key_menu_confirm && key != 'y')
	return;
    if (!netgame)
    {
	if (gamemode == commercial)
	...
	if (messageNeedsInput)
        {
            if (key != ' ' && key != KEY_ESCAPE
             && key != key_menu_confirm && key != key_menu_abort && key != 'y')
            {
                printf("[doom] M_Responder messageNeedsInput ignore key=%d('%c') confirm=%d('%c') abort=%d('%c')\n",
                       key, key, key_menu_confirm, key_menu_confirm, key_menu_abort, key_menu_abort);
                return false;
            }
	}
