...
void I_Quit (void)
{
    ...
    while (entry != NULL)
    {
        printf("[doom] I_Quit executing exit func %p\n", (void*)entry->func);
        entry->func();
        printf("[doom] I_Quit exit func %p done\n", (void*)entry->func);
        entry = entry->next;
    }

    printf("[doom] I_Quit finished callbacks, calling exit(0)\n");
    exit(0);
