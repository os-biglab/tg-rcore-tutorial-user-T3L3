...
{
    // Doom full quit sequence may include a blocking ENDOOM screen
    // (waiting for key). In this environment we have no reliable
    // interactive host keyboard once exiting, so skip it.

    (void)show_endoom;
    (void)main_loop_started;
    (void)screensaver_mode;

    printf("[doom] D_Endoom bypassed to avoid hang\n");
    return;
